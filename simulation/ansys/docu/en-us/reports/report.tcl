...
#            catch {ans_sendcommand )/SHOW,OFF} err
   ...
   proc outputcapture { caption ansCmd } {
      ...
      while {[gets $fileOut line] >= 0} {
         ...
         set printme 1
#         NOTE: uncomment these checks to filter headings in non-verification batch runs
#         if {[string match {1} $line]} {
#            set printme 0
#         }
#         if {[string match { ***** ANSYS - ENGINEERING ANALYSIS SYSTEM  RELEASE 12.0 *****} $line]} {
#            set printme 0
#         }
#         if {[string match { ANSYS Multiphysics/LS-DYNA                        } $line]} {
#            set printme 0
#         }
#         NOTE: suppress headings in verification batch runs
         if {[string match {   *****MAPDL VERIFICATION RUN ONLY*****} $line]} {
            set printme 0
         }
         if {[string match {     DO NOT USE RESULTS FOR PRODUCTION} $line]} {
            set printme 0
         }
         if {$printme == 1} {
      ...
      if {$ReportArray(menu) && !$ReportArray(batchCapture)} {
         ...
         if {![ans_getvalue graph,,erase] \
            ...
            tk_messageBox -type ok \
               ...
               MAPDL or ansys::report::imageoverlay must be used."
            update idletasks
         }
         ::apdl::noprint 1
...
#                  catch {ans_sendcommand )/SHOW,OFF} err
         ...
         1 {
            ...
            set ShowArray(mode) [ans_getvalue graph,,display]
            ...
            catch {ans_sendcommand )/SHOW,$format,,$ShowArray(mode),8} err
         }
      }
   }
   # END OF IMAGE PROCS }
